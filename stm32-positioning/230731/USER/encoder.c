...
Vector2f Displacement,Displacement_expect,Displacement_err,Last_Displacement;					//坐标
...
void Velocity_Fusion(void)
{
	...
	//TCoord_Radian=Angle_z_temp*M_PI/180+atan2(Diff_Distance1,Diff_Distance2);//计算单位时间内的位移与坐标轴夹角弧度
	TCoord_Radian=Angle_z_temp*M_PI/180-M_PI/4+atan2(Diff_Distance1,Diff_Distance2);//计
	Displacement.x+=Dis_Journey*cos(TCoord_Radian)*10;//x坐标 
	Displacement.y+=Dis_Journey*sin(TCoord_Radian)*10;//y坐标

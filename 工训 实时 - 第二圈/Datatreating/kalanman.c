...
//    Accel_x=imu660ra_acc_transition(accel[0]);
//    Accel_y=imu660ra_acc_transition(accel[1]);
//    Accel_z=imu660ra_acc_transition(accel[2]);
//    Gyro_x=imu660ra_gyro_transition(gyro[0]);
//    Gyro_y=imu660ra_gyro_transition(gyro[1]);
//    Gyro_z=imu660ra_gyro_transition(gyro[2] - Gyro_Z_Drift);
    Angle_x_temp=atan2(Accel_y,Accel_z)*180/3.14;     //计算倾角，转换单位为度
    Angle_y_temp=atan2(Accel_x,Accel_z)*180/3.14;     //计算倾角，转换单位为度
//    Angle_z_temp+=Gyro_z/16.4*0.015*2*1.05;'

			Angle_z_temp+=Gyro_z/16.4*0.003;


    if(Angle_z_temp> 180.0)  Angle_z_temp =-180.0;
    if(Angle_z_temp<-180.0)  Angle_z_temp =180.0;

   // Angle_X_Final=Kalman_Filter_x(Angle_x_temp,-Gyro_x);//卡尔曼滤波
    Angle_Y_Final=Kalman_Filter_y(Angle_y_temp,-Gyro_y);
//    Gyro_x = constrain_int(Gyro_x,-120,120);
  // Angle_Z_Final=Kalman_Filter_y(Angle_z_temp,-Gyro_z);
...
float Complementary_Filter_x(void)
{
     ...
     Accel_x = AccX;//x轴加速度值暂存
     Accel_y = AccY;//y轴加速度值暂存
     Accel_z = AccZ;//z轴加速度值暂存
     Gyro_x  = WX;//x轴陀螺仪值暂存
     Gyro_y  = WY;//y轴陀螺仪值暂存
     Gyro_z  = WZ;//z轴陀螺仪值暂存
